...
const UserMsg = ({ message }) => {
  return (
    <div className={`rc-relative display-flex rc-m-10 rcMsgContainer
      ...
      <img
        src={message.sender === sender.CUSTOMER ? customerAvatar : avatar}
        className={`avatar rc-absolute ${message.sender === sender.CUSTOMER 
        ? "rc-align-r" : ""}`} 
        alt="avatar" />
      <div className={`rc-p-10 rc-word-wrap bubble rc-br-10 ${message.sender === sender.CUSTOMER 
      ? "chat-bg-sec rc-m-r-50" : "chat-bg-primary rc-m-l-50 "}`}>
        {message.text}
      </div>
    </div>
  );
